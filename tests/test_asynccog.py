...
AsyncCOGReader = type("AsyncReader", (AsyncReader,), {"reader": COGReader})
router = factory.TilerFactory(reader=COGReader, router_prefix="cog").router


@pytest.fixture(autouse=True)
def async_app(set_env) -> TestClient:
    """Create App."""
    app = FastAPI()
    app.include_router(router, prefix="/cog")
    return TestClient(app)


@patch("rio_tiler.io.cogeo.rasterio")
def test_bounds(rio, async_app):
    """test /bounds endpoint."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/bounds?url=https://myurl.com/cog.tif")
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_info(rio, async_app):
    """test /info endpoint."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/info?url=https://myurl.com/cog.tif")
    assert response.status_code == 200
    body = response.json()
    assert len(body["bounds"]) == 4
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_metadata(rio, async_app):
    """test /metadata endpoint."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/metadata?url=https://myurl.com/cog.tif")
    assert response.status_code == 200
    body = response.json()
    assert len(body["bounds"]) == 4
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_wmts(rio, async_app):
    """test wmts endpoints."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/WMTSCapabilities.xml?url=https://myurl.com/cog.tif")
    assert response.status_code == 200
...
def test_tile(rio, async_app):
    ...
    response = async_app.get(
        "/cog/tiles/8/87/48.npy?url=https://myurl.com/cog.tif&nodata=0"
    )
    assert response.status_code == 200
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_tilejson(rio, async_app):
    """test /tilejson endpoint."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/tilejson.json?url=https://myurl.com/cog.tif")
    assert response.status_code == 200
    body = response.json()
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_preview(rio, async_app):
...
@patch("rio_tiler.io.cogeo.rasterio")
def test_point(rio, async_app):
    """test /point endpoint."""
    rio.open = mock_rasterio_open

    response = async_app.get("/cog/point/-56.228,72.715?url=https://myurl.com/cog.tif")
    assert response.status_code == 200
